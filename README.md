...
- 位置包括手动区块、正文前、正文后，以及第 1–20 段后（默认第 3 段）；区块和短代码直接引用 Promotion ID。
...
- 正式投放由服务端判断发布状态、标准内容范围和时间；设备可见性使用 CSS 断点，避免全页缓存按 User-Agent 串设备。
...
当前开发线已实现 [ADR 007](docs/decisions/007-canonical-editorial-scope.md) 的互斥 content scope：全部标准文章/页面、全部文章、全部页面、直接匹配所选 Core 分类/标签的文章，或显式选择的标准文章/页面；显式 ID 排除始终优先。0.2 收口仍有一项待办：完善手动区块引导并解释现有桌面/移动断点，不把这一步描述为已完成。
...
Playground 会验证单 Promotion 数据模型、typed meta、发布预检、五种 canonical content scope、真实 Core 分类/标签直接关系与动态变更、term fail-closed、时间/设备规则、区块/短代码/自动位置、顶层区块与 Classic 段落锚点、管理员/订阅者/匿名 REST 边界、绑定 Promotion 的预览 nonce、时区与排期边界、无 Placement/Options/自定义表以及显式卸载清理。真实编辑器、列表快捷操作与主题页面交互仍需在 Local 中做浏览器验收。
