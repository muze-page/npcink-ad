...
当前开发线已完成 [ADR 005](docs/decisions/005-controlled-delivery-expansion.md) 的受控 0.2 范围：[ADR 006](docs/decisions/006-paragraph-anchor-delivery.md) 的第 N 段后位置、[ADR 007](docs/decisions/007-canonical-editorial-scope.md) 的互斥 content scope，以及 [ADR 008](docs/decisions/008-manual-placement-and-device-guidance.md) 的显式手动入口和固定设备说明。手动 scope 只展示 `all | selected`，仍需单独插入区块或短代码，ID 排除始终优先，缺块核验不阻止发布。0.2 版本号、更新日志、最终打包与发布签核仍属于 0.2 收尾，本文不提前宣称 0.2 已发布。
