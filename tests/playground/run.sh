...
PLUGIN_VERSION=$(sed -n '/^[[:space:]]*\*[[:space:]]*Version:/ { s/^[[:space:]]*\*[[:space:]]*Version:[[:space:]]*//; p; q; }' "$PROJECT_DIR/magick-ad.php")

if [ -z "$PLUGIN_VERSION" ]; then
	echo 'Could not read the plugin Version header from magick-ad.php.' >&2
	exit 1
fi

PLUGIN_ZIP="${PLUGIN_ZIP:-$PROJECT_DIR/dist/magick-ad-$PLUGIN_VERSION.zip}"
...
TEMP_DIR=$(mktemp -d "${TMPDIR:-/tmp}/magick-ad-playground.XXXXXX")
cleanup() {
	rm -rf "$TEMP_DIR"
}
...
cp "$PLUGIN_ZIP" "$TEMP_DIR/magick-ad.zip"
...
npx -y "@wp-playground/cli@$PLAYGROUND_CLI_VERSION" run-blueprint \
	...
	--mount="$TEMP_DIR/results:/wordpress/wp-content/magick-ad-smoke-results" \
	--wp="$WP_VERSION" \
	--verbosity=normal
...
if [ "$ACTUAL_STATUS" != 'MAGICK_AD_SMOKE_OK' ]; then
