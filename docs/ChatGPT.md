...
可以，而且我建议你同时提供 3 条“插入入口”（面向不同用户/场景），并且它们底层都调用同一个渲染函数，这样功能一致、维护成本最低：1. 区块（Block）：现代 WP 首选（Gutenberg / FSE）2. 短代码（Shortcode）：兼容经典编辑器/复制粘贴场景3. 主题模板函数（Template Tag / PHP API）：给主题开发者、放在模板任意位置
...
• 解决方案通常是：1. “稳定随机”（基于会话/用户桶），或者2. “前端轮播/随机”（JS 在客户端决定）
...
推荐你先落地的最小版本（投入最小、覆盖最大）1. ✅ magick-ad/slot 动态区块（可选 adId 或 slot） ￼ 2. ✅ [magick_ad] 短代码（支持 id/slot + 少量参数） ￼ 3. ✅ magick_ad_get() / magick_ad_the() 模板函数（slot 优先）4. ✅ [magick_ad_trigger]（给弹窗用） ￼
...
当 show_position === 'node' 时，新增一组 options（建议放在 options 里）：1. 定位方式（只做 ID / class，先别开放完整 CSS selector）
...
节点投放面板（建议字段顺序）1. 定位方式：ID / Class（单选）2. 节点值：TextControl（提示：ID 不带 #，Class 不带 .）3. 插入方式：SelectControl（append/prepend/before/after）4. 匹配多个节点：SelectControl（first / nth / all）5. 第 N 个：RangeControl（仅 nth 时显示）6. 找不到节点时：SelectControl（hide / footer）7. ✅ 紧凑模式：ToggleControl（默认开启）
...
4.1 服务器端（PHP）1. 在页面底部输出一个隐藏容器（只输出一次）：
...
核心流程 1. 找到 stash 2. 遍历 stash 内所有节点广告单元 3. 计算 selector：
• type=id → #${value}
	•	type=class → .${value} 4. 查找目标节点：
• first → document.querySelector(selector)
• all → document.querySelectorAll(selector)
• nth → document.querySelectorAll(selector)[index-1] 5. 执行插入（推荐用 insertAdjacentElement）：
• append → beforeend
• prepend → afterbegin
• before → beforebegin
• after → afterend 6. compact 模式：给广告单元加一个 class，例如 .magick-ad-placement--node-compact，CSS 里把 margin 变为 0 7. 插入成功后：从 stash 移除原节点（或保留但标记已处理）
...
你要实现 MVP，大致需要改这些地方：1. PHP：Settings.php
...
下面是基于你现有项目结构的 “节点广告（按 ID / class 插入）”MVP 实施清单。我按“最少改动、易回滚”的方式整理，适合你当前开发期直接大胆改造。

一、数据模型与校验（后端）
目标：让 placement 支持 node，并有严格字段校验。

Settings.php（或你的 sanitize/validate 文件）
show*position 增加 node 选项
新增字段（建议放在 options）：
node_target_type: id|class
node_target_value: string
node_insert: append|prepend|before|after
node_match: first|nth|all（MVP先 first|nth）
node_index: number (nth 时必填)
node_fallback: hide|footer
node_compact: boolean
sanitize 建议：
node_target_value 用严格正则 ^[A-Za-z*][A-Za-z0-9_-]\*$
validate：
当 show_position === 'node' 时强制检查上述字段
二、后台 UI（Admin）
目标：展示位置里新增“指定节点”，并出现节点配置面板。

options.js
在位置下拉里追加 { label: '指定节点（ID / class）', value: 'node' }
AdsConfig.js
当 show_position === 'node' 时，显示一个 PanelBody「节点投放」
UI 字段顺序（建议）：
定位方式 (ID / Class)
节点值（提示：ID 不带 #，Class 不带 .）
插入方式（append/prepend/before/after）
匹配策略（first / nth / all）
第 N 个（RangeControl，仅 nth）
找不到节点时（hide/footer）
紧凑模式 Toggle
三、前台渲染（PHP）
目标：node 广告不直接渲染到可见区，先放到 stash。

Frontend.php
在页面底部输出一次隐藏 stash：

<div id="magick-ad-stash" class="magick-ad-stash" style="display:none"></div>
所有 show_position === 'node' 广告 输出到 stash，并带 data-attributes：
data-ad-node-type
data-ad-node-value
data-ad-node-insert
data-ad-node-match
data-ad-node-index
data-ad-node-fallback
data-ad-node-compact
继续复用现有 build_ad_markup()，避免 JS 拼 HTML。
四、前端 JS（注入逻辑）
目标：DOMReady 后将 stash 广告插入到目标节点。

magick-ad-track.js（或你的前端脚本）
在现有 observer 初始化之前新增 placeNodeAds()：
找到 stash
遍历其中的 .magick-ad-unit
根据 type/value 生成 selector
first / nth / all 寻址
insertAdjacentElement 实现 append/prepend/before/after
compact 模式：加 class magick-ad-placement--node-compact
插入成功后移除原节点 or 标记 data-node-inserted="1"
防重复插入：检测 data-node-inserted
如果 match=all，需要 cloneNode(true) 再插入
五、CSS 微调
目标：避免 node 插入到 header/sidebar 后被撑爆。

frontend.css（或现有前端样式）
添加紧凑样式：
.magick-ad-placement--node-compact {
margin: 0 !important;
}
如需：限制最大宽度/继承父级
六、调试/日志（可选但建议）
...
调试输出（已有 debug 系统可复用）
debug 开启时 console 输出：
selector
match 数量
插入方式
fallback 触发原因
若目标是 <p> 或 void element（img/input），append/prepend 时自动降级并记录原因
MVP 范围建议（先做这些）

目标类型：ID / class
插入方式：append / prepend / before / after
匹配：first / nth
fallback：hide / footer
compact：默认开启
不做 MutationObserver
我可以直接实施
如果你确认，我可以按上面的顺序把改造全部落地（含 UI + PHP + 前端 JS + CSS）。
你只需要回复：
“开始实施节点投放 MVP” 或者说你想先做哪一部分（例如“先做后台 UI”）。
