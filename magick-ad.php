...
function magick_ad_register_template_cpt() {
...
add_action('init', 'magick_ad_register_template_cpt');
...
function magick_ad_register_rest_routes() {
    ...
    register_rest_route('magick-ad/v1', '/templates', array(
        'methods' => 'GET',
        'callback' => array('Magick_AD_Templates', 'rest_list'),
        'permission_callback' => 'magick_ad_can_manage',
    ));
    register_rest_route('magick-ad/v1', '/templates', array(
        'methods' => 'POST',
        'callback' => array('Magick_AD_Templates', 'rest_create'),
        'permission_callback' => 'magick_ad_can_manage',
    ));
    register_rest_route('magick-ad/v1', '/templates/import', array(
        'methods' => 'POST',
        'callback' => array('Magick_AD_Templates', 'rest_import'),
        'permission_callback' => 'magick_ad_can_manage',
    ));
}
...
class Magick_AD_Templates {
    ...
    public static function rest_create(WP_REST_Request $request) {
        ...
        $clean_type = Magick_AD_Engine::sanitize_choice(
            $type,
            array('html', 'image', 'video', 'block'),
            'html'
        );
    ...
    private static function sanitize_template_data($type, $data) {
        $data = is_array($data) ? $data : array();
        if ($type === 'image') {
    ...
    private static function maybe_sideload_image(&$data) {
