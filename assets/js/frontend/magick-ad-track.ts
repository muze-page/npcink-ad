import type { TrackBatchItem, TrackConfig, TrackPayload } from '../types';

(() => {
    const config =
        ((window as unknown as { MagickADTrack?: TrackConfig })
            .MagickADTrack as TrackConfig) || {};
    ...
    const resolveAdElement = (options: Record<string, unknown>) => {
    ...
    const buildItemPayload = (payload: TrackPayload): TrackBatchItem => {
        const item: TrackBatchItem = {
            ad_id: payload.adId,
            event: payload.event,
        };
        if (payload.sig) {
    ...
    const buildBatchPayload = (items: TrackPayload[]) => {
    ...
    const trackCustomEvent = (event: unknown, options: Record<string, unknown> = {}) => {
